...
type Props = {
    ...
    variant: Variant;
    onCancel: () => void;
    onConfirm: (opts?: { sampleCount?: number }) => void;
...
export default function ConfirmSendDialog({
    ...
    useEffect(() => {
        ...
        const onKey = (e: KeyboardEvent) => {
            ...
            if (e.key === "Enter") onConfirm(variant === "test" ? { sampleCount } : undefined);
        };
        window.addEventListener("keydown", onKey);
        setTimeout(() => primaryRef.current?.focus(), 0);
        return () => window.removeEventListener("keydown", onKey);
    }, [open, onCancel, onConfirm, sampleCount, variant]);

    useEffect(() => {
        setSampleCount(defaultSampleCount);
    }, [open, defaultSampleCount]);
                ...
                <div className="p-4">
                    ...
                    <div className="mt-4 flex items-center justify-end gap-2">
                        ...
                        <div
                            ...
                            onClick={!isSubmitting ? () => onConfirm(variant === "test" ? { sampleCount } : undefined) : undefined}
                            onKeyDown={(e) =>
                                (e.key === "Enter" || e.key === " ") &&
                                !isSubmitting &&
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <span>{isSubmitting ? "Sending…" : variant === "real" ? "Yes, send" : "Send test"}</span>
                            </div>
                        </div>
                    </div>
                </div>
